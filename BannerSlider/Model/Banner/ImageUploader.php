...
class ImageUploader
{
     ...
     */
    public function __construct(\Magento\MediaStorage\Helper\File\Storage\Database $coreFileStorageDatabase, \Magento\Framework\Filesystem $filesystem, \Magento\MediaStorage\Model\File\UploaderFactory $uploaderFactory, \Magento\Store\Model\StoreManagerInterface $storeManager, \Psr\Log\LoggerInterface $logger, $baseTmpPath = 'magentovlogs/tmp/banners_slider', $basePath = 'magentovlogs/banners_slider', $allowedExtensions = ['jpg', 'jpeg', 'gif', 'png'])
    {
    ...
    public function moveFileFromTmp($imageName)
    {
        ...
        {
            if ($this->getFileInfo()
                ->isExist($imageName, $this->baseTmpPath))
            {
                $this
                    ->coreFileStorageDatabase
                    ->copyFile($baseTmpImagePath, $baseImagePath);
                $this
                    ->mediaDirectory
                    ->renameFile($baseTmpImagePath, $baseImagePath);
            }
        }
        catch(\Exception $e)
        {
    ...
    public function saveFileToTmpDir($fileId)
    {
        ...
        $result = $uploader->save($this
            ->mediaDirectory
            ->getAbsolutePath($baseTmpPath));
        unset($result['path']);
    ...
    public function deleteImage($imageName, $type = 'dir')
    {
        ...
        if ($this->getFileInfo()
            ->isExist($imageName, $basePath))
        {
            $this->getFileInfo()
                ->deleteFile($imageName, $basePath);
        }
    }
}
